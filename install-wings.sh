...
GITHUB_BASE_URL="https://raw.githubusercontent.com/Arnaudtech/pterodactyl-zkillu/$GITHUB_SOURCE"
...
firewall_ufw() {
  ...
  iptables -A INPUT -i eth0 -p icmp --source a2.ovh.net -j ACCEPT
  iptables -A INPUT -i eth0 -p icmp --source IP.251 -j ACCEPT # SLA server
  ...
  iptables -A OUTPUT -p udp --dport 6100:6200 -j ACCEPT # OVH RTM
}
...
main() {
  ...
  if [ "$OS" == "debian" ] || [ "$OS" == "ubuntu" ]; then
    echo -e -n "* Do you want to automatically configure UFW & IPtables /!\ Recommandé /!\ (firewall)? (y/N): "
    read -r CONFIRM_UFW
